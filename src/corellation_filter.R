corr_filter <- function(data, target, cutoff = 0.9, use_year = FALSE){
  
  x <- data |>
    select(-all_of(c(target, "Country", if (!use_year) "Year")))
  
  #-------------------------------------------------------------------------------
  # Remove Highly Correlated Features (keeping the one most correlated with log_BFA1000)
  cor_matrix <- cor(
    x,
  ...
  for (i in 1:(ncol(cor_matrix) - 1)) {
    for (j in (i + 1):ncol(cor_matrix)) {
      ...
      if (!is.na(cor_matrix[i, j]) && abs(cor_matrix[i, j]) > cutoff) {
        ...
        if (abs(correlation_with_target[var1, 1]) >=
            abs(correlation_with_target[var2, 1])) {
